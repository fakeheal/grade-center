...
export async function loader({ request, params }) {
    ...
    const getDirectorRequest = await fetch(`${apiConfig.baseUrl}/parents/${params.id}`);
    const parent = await getDirectorRequest.json();
    return { schools, parent };
}
...
export async function action({ request, params }){
    ...
    const response = await fetch(`${apiConfig.baseUrl}/parents/${params.id}`, {
        method: 'PUT',
        headers: {
...
export default function Edit({ loaderData, actionData }) {
    ...
    const { schools, parent } = loaderData || {};
    return (
        <div>
            <DirectorForm director={parent} schools={schools.content} errors={errors}></DirectorForm>
            {errors?.general && (
